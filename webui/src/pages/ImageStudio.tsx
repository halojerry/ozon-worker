 ...
 *
 * C3b 冻结映射（worker utils/image_gen_plan.py，T7b 对齐）：
 *   白底图→white_bg / 场景图→scene(0-3) / 卖点图→main_image
 *   细节图→detail / 对比图→comparison / 社交证明→social_proof
 *   多角度→multi_angle / 材质图·尺寸图→无现成节点（v1 置灰）
 *
 * 一键生成 = 余额门（/auth/verify，balance_insufficient 阻止）
 *   → 确认弹窗（预计消耗 = plan 总张数 × 1 次调用）→ 逐槽位
 *   POST /tasks/{id}/images/{slot}/regen（version++ 新 URL）。
 *
      ...
      if (identity.kind === 'task') {
        ...
        if (snap) {
          try {
            ...
            if (parsed.selling) setSelling({ ...EMPTY_SELLING, ...parsed.selling })
          } catch {
            /* 快照损坏忽略，用默认值 */
          }
        }
  ...
  return (
    <div className="page">
      <header className="page-header">
        <h1 className="page-title">生图工作台</h1>
        <span className="page-badge">T13</span>
        <span className="studio-task-meta mono">{identity.kind === 'task' ? `任务 ${taskId.slice(0, 8)}` : `草稿 ${draftId.slice(0, 8)}`}</span>
      </header>
