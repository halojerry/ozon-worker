        ...
        <SectionPageLayout.Content>
          <div className='mx-auto flex w-full max-w-7xl flex-col gap-4 sm:gap-5'>
            ...
            <InvitationCard
              ...
              referralTreeRows={referralTreeRows}
              referralTreeSummary={referralTreeSummary}
              referralInviteDepth={referralInviteDepth}
              setReferralInviteDepth={setReferralInviteDepth}
              refreshReferral={refreshReferral}
              onOpenTransfer={() => setTransferDialogOpen(true)}
              affQuota={user?.aff_quota ?? 0}
              renderQuota={(q: number) => `${(q / (status?.quota_per_unit || 500000)).toFixed(2)} ${status?.display_in_currency ? 'USD' : '额度'}`}
            />
          </div>
        </SectionPageLayout.Content>
      </SectionPageLayout>
