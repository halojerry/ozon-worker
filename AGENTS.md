...
> 2026-09-08。已发版（VERSION 四源 0.70.0；v0.69.0 未单独 tag 随本版发出）。发版
> gate 已通过（2026-09-08 本地真实 discover ≥3 单：2 单真 approved、L1 面包屑仲裁
> 命中竞品真实类目、零假 completed；重提两卡终态 fail 属已知问题类，见 CHANGELOG
> 0.70.0 gate 终态补记）。生产实证底稿：`data/official_feedback_20260907.md`
...
- **重量硬下限**：Ozon 契约下限 10g——worker normalizer 将 (0,10)g 视同缺失走
  竞品/默认 100g 兜底（**不 ×1000**）；skill `cloud_probe._sanitize_weight_g` 抓取层
  出口守卫（1688 包装表无单位裸数字 kg 语义按克产出 1g 是 A3 拒单根因）。**改重量链
  前必读 normalizer 的 v0.68.1 注释**——legacy「真实 3g 保持」语义已废（保持=必拒单）。
- **Step 6.5 豁免判定 `_step65_consistency_exempt`**：Skill/权威 L0/R2b 已确认三类
  豁免一致性重配；普通 L1 不豁免（A2 型救回保持）。改 Step 6.5 前先读该纯函数。
- **回归再发现待修**：①A3 残余拒单=估算尺寸 60×45×30mm 过小（Ozon 重量×体积交叉
  校验，报错误导性只报 weight range）；②R4 换类目目标无域守卫（A4 被重配到除草剂，
  标题「除草」关键词牵引）。见 CHANGELOG 0.68.1 已知问题。
