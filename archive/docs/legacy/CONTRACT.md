...
> **原则：一个 1688 item = 一个 Ozon 产品卡，不上变体。**

Skill 层自动完成变体折叠（`_collapse_variants_to_single()`）：

- **过滤引流款**（已有 `_filter_bait_and_custom_skus`）
- **数量变体**（如 1只装/5只装）→ 筛选「1只装」变体
- **颜色/尺寸变体** → 取中位数价格的变体
- **混合变体**（颜色×数量）→ 先筛 1只装，再取中位数
- **采购成本** = 代表变体价格 + 1688 国内运费（`shipping.freightCny`）
- **标题**：不加颜色/数量后缀，保持原产品名
- **主图**：1688 主图 + 标准白底图 + 营销图

Worker 收到的信封 `variants` 最多 1 个元素，走现有单产品路径，无需额外改动。
