...
- follow：6/6 approved（f1 + 波次 5；典型 worker 侧 60s/卡）。
- discover：599d15ae → 6386240597 approved；25 品采集+运营指标 4m15s。
...
- L0 弱档学习串类目（f1 学习行盖过 jieba 正确匹配，筷子篮被拦——弱档 L0 应
  让位高分文本匹配）；`discover --non-interactive --auto-submit` 仍弹确认框
  自动取消；0 候选不写导出文件；discover aibuy 图搜空 → CDP 兜底 ~17s/品。
...
### 生图串图根治 + 参考图两条线（同版收录，fix/image-ref-pollution-clean）
