...
| `discover-task` | 任务式全自动目标驱动选品（无人值守） | `--keyword/--url [--target-count 达标数] [--max-scan 300] [--filter-profile ai] [--filters JSON规则文件（区间/品牌/价格/发货模式，FBS 含 rFBS，子串匹配，与库内口径一致，同名键覆盖 ai 默认）] [--base-filter] [--min-price/--max-price/--brand-filter] [--match-limit =目标×3] [--to-box\|--auto-submit 二选一] [--dry-run] [--resume] [--expend-shop N 拓店（--url 须商品页种子，竞品卖家按价排序一跳店铺展开，预算 max(N×4,60)，与 --keyword 互斥；评分过滤待真实卖家评分数据源）]` | 缺省干跑；`--to-box` 写采集箱（WebUI 认领后上架）；`--auto-submit` 直上管线（必须确认）；状态落 `data/discovery/tasks/`；粗筛池耗尽未达标会如实报缺口 | "自动采集/无人值守/任务式跑 N 个" / "以这个商品为种子拓店" |
