...
用户输入
 ...
 │    "无限找货/源头挖掘/顺着卖家找/从同行挖货" → 【管线 C】discover 裂变选品（`--fission`）
...
- ⚠️ **裂变选品（discover --fission，v0.31）**：以「跟卖卖家 = 最好的选品工具」——
  从种子商品出发 → 抓跟卖卖家（同行）→ 扫卖家店铺其他产品 → 再发现 → BFS 扩散，
  同行店铺里源源不断的产品 = 已被市场验证的货源。用法：
  - `discover --keyword <品类词> --fission`：先正常采集种子（管线 C 阶段①②），
    再裂变展开（种子商品 → 跟卖卖家 → 卖家店铺产品）
  - 可选参数：`--max-depth 2`（默认 2，>3 需 `--allow-depth-3` 显式，指数爆炸）、
    `--max-total-products 300`（候选总量上限，**主成本控制**）、`--time-budget 600`、
    `--max-products-per-seller 15`、`--non-interactive`（层间不询问继续）
  - ⚠️ 裂变是**慢操作**（卖家页串行导航 + what_to_sell 逐 SKU 限速），跑一次约 10-60 分钟
  - ⚠️ 裂变候选带 `source_chain`（来源链路：种子→卖家→产品），选中产品时可查看出处
  - ⚠️ 裂变依赖 widget API 拿跟卖卖家（需 Ozon 页面正常加载 + 登录态）；部分产品
    反爬偶发失败 → 自动降级跳过，不影响整体
