...
- **参数补充**：`--filters <JSON规则文件>`（区间/品牌/价格/发货模式，FBS 含 rFBS，子串匹配，同名键覆盖 ai 默认）、`--min-price/--max-price/--brand-filter`、`--expend-shop N` 拓店（`--url` 须商品页种子，竞品卖家评级优先，预算 max(N×4,60)，与 `--keyword` 互斥）、`--dry-run`（强制干跑）、`--wait`（auto-submit 出口逐个等终态）
