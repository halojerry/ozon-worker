...
**首次使用（v0.28.4）**：`check` 会启动工具专用 Chrome（独立 profile，不影响日常浏览器），
未登录 1688 时自动打开登录页 + Ozon Seller 卖家后台（discover 运营指标需要），
交互环境等待登录后按 Enter 继续。**登录后浏览器保持常驻复用**——后续命令直接复用
同一实例（不再每次弹新窗口）；用户手动关闭后，下次命令检测到 CDP 不可用会自动重启。
