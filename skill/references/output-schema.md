...
> **重量语义（v0.68.1，核对信封/汇报时必读）**：Ozon 契约下限 10g——skill 抓取层
> 出口守卫 `_sanitize_weight_g` 把 (0,10)g 视同缺失走竞品重量/默认 100g 兜底
> （**不 ×1000**）。历史上 1688 包装表无单位裸数字按克产出 1g 是 A3 拒单根因
> （Ozon 重量×体积交叉校验）；若看到 1-9g 的重量出现在汇报/信封里即为异常，先查货源页包装表。

### 价格三语义（汇报/核对价格时必读，v0.70 口径）

| 价格 | Ozon 语义 | 在哪设 |
|---|---|---|
| 日常价 price | 商品卡显示的售价 | **上架时**（import item.price） |
| 划线价 old_price | 折扣前价格（卡上划线；无折扣传 0） | **上架时**（item.old_price） |
| 促销底价 min_price | 参加 Ozon 促销（Акции）时价格不会跌破的防线 | 提交后按 promo_price 经 /v1/product/import/prices 补送 |
| 促销价 | 不写在商品卡上，由促销活动折扣规则决定 | Ozon 卖家后台 促销(Акции) 模块 |

汇报或核对价格时不要把促销价说成商品卡字段——它属于促销活动；系统里的
promo_price 是 min_price 底线语义。采集箱「AI 预组装」的 estimated_pricing
是展示性估算（日常/划线/底线三档），提交后仍由 worker 按店铺配置精算。
