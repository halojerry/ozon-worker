...
def _generate_import_stubs(dist_dir: Path, compile_files: list[str]) -> None:
    ...
    for py_file in compile_files:
        ...
        stub_content = (
            ...
            '# ⚠️ Rosetta 2 兼容：darwin-x86_64 下 platform.machine() 伪装返回 arm64，\n'
            '# 但打包目录可能是 darwin-x86_64（Fix platform dir 步骤改名）——两个 arch 都试\n'
            'if _plat == "darwin" and not _native_dir.is_dir():\n'
            '    _alt_arch = "darwin-x86_64" if _pm.machine().lower() == "arm64" else "darwin-arm64"\n'
            '    _alt_dir = _Path(__file__).resolve().parent / "' + native_rel.replace('\\', '/') + '" / _alt_arch\n'
            '    if _alt_dir.is_dir():\n'
            '        _native_dir = _alt_dir\n'
            '        _plat_name = _alt_arch\n'
