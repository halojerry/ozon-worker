...
def scene_3_gen_node(state: Scene3Input, config: RunnableConfig, runtime: Runtime[Context]) -> Scene3Output:
    ...
    """
    ctx = runtime.context
    # ✅ T7b: image_gen_plan 前置条件（plan 无 scene_3 → 直接跳过，不调生图 API）
    if not slot_enabled(config, "scene_3", state):
        logger.info("image_gen_plan 未选择 scene_3，跳过生图")
    ...
    # slot_scene_context=scene_context_3 保证三张场景图差异化（F6，不覆盖）
