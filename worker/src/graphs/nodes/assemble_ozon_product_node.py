...
def _step65_consistency_exempt(match_layer: str, r2b_confirmed: bool,
                               l0_exempt: bool, box_reviewed: bool = False) -> bool:
    ...
    - ✅ v0.70 采集箱即权威（box_reviewed）：用户在采集箱编辑/采纳过类目，
      提交内容=人工审核结果，管线不重决策（extensions.box_reviewed，由
      submit_draft 注入）。
    普通 L1 不豁免——A2 型（三角头巾→遮阳帽）Step 6.5 救回语义保持。
    """
    return match_layer == "Skill" or r2b_confirmed or l0_exempt or box_reviewed
...
def assemble_ozon_product_node(
    ...
    if source_category:
        ...
        if cat_terms:
            ...
            # v0.40: 上级词单独存（回退通道，不进主搜索词——防 v0.34 稀释回归）。
            # 末级词在 Ozon 无对应时（工业品"绝缘子"→0 命中）用上级词（"电工电气"）
            # 单独搜索，可命中 Ozon 大类（"建筑和装修 > 电气" 48 条）。
            parent_terms = [t for t in reversed(cat_terms[:-1]) if t not in _GENERIC_WORDS][:2]
    ...
    # v0.40: 末级词/标题 0 候选 → 用 1688 上级类目词回退（工业品如"绝缘子"
    # Ozon 无对应，但上级"电工电气"→ Ozon"电气"大类可命中）
    if not candidates and parent_terms:
    ...
    # ✅ v0.67.1 wave④: 移除「低置信时用 parent 词重开候选池」通道——姊妹词
    # （帽子/头巾 的「头巾」）借单 token 假满分抢占后，此通道把诚实候选整池丢弃
    # （A2/A3/A7/A8 四帽全落三角头巾的帮凶）。低置信场景改走既有 LLM fallback 链
    # + Step 6.5 一致性重配；0 候选时的 parent 回退通道（搜索 0 命中处）保留。
    ...
    # ✅ v0.67.1 wave④: 日志标签用真实 matcher（原硬编码 pg_trgm 误导取证）
    logger.info(f"   ✅ 类目匹配 ({best.get('matcher', 'pg_trgm')}): [{best['description_category_id']}/{best['type_id']}] {best['full_path']} (sim={best.get('similarity', 0):.3f})")
    ...
    if not l0_hit:
        ...
        #  它是语义大类回退而非字面匹配）
        if parent_fallback_used:
            _pf_cand = candidates[0]
            category_result = {
                "description_category_id": _pf_cand["description_category_id"],
                "type_id": _pf_cand["type_id"],
                "category_path": _pf_cand["full_path"],
                "confidence": "medium",
                "reason": f"parent_category_fallback (sim={_pf_cand.get('similarity', 0):.3f})",
                "similarity": _pf_cand.get("similarity", 0),
                "matcher": _pf_cand.get("matcher", "pg_trgm"),
            }
            # ✅ v0.67.1 wave④: parent 通道无 overlap 豁免佐证——置信度封顶 0.5，
            # 保证后续 Step 6.5 一致性复核/R2b 不被 sim=1.0 的 parent 假满分白嫖
            match_confidence = min(_confidence_from_sim(_pf_cand.get("similarity")), 0.5)
            logger.info(f"   ✅ 上级类目回退采用: [{_pf_cand['description_category_id']}/{_pf_cand['type_id']}] {_pf_cand['full_path']} (sim={_pf_cand.get('similarity', 0):.3f}, conf={match_confidence:.2f})")
        else:
