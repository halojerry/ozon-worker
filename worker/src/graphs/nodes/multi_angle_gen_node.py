...
def multi_angle_gen_node(state: MultiAngleInput, config: RunnableConfig, runtime: Runtime[Context]) -> MultiAngleOutput:
    ...
    # fix/image-ref-pollution R2 参考图两条线（语义同 white_bg_gen_node）：
    #   跟卖=竞品原图优先 > 1688 货源图；1688 直上=仅货源图；缩略图恒拒。
    ...
    try:
        ...
        # fix/image-ref-pollution: 无合格参考图不硬生成（对齐 main_image v0.14 B5）
        if not ref_images:
            logger.warning("无合格参考图（原图缺失或全为缩略/竞品图），跳过多角度图生成")
            return MultiAngleOutput(multi_angle_image=None)

        # ✅ 调用统一mxou API（正确参数: images/aspectRatio/replyType）
        logger.info("正在调用图片生成API...（timeout=180s）")
        image_url = call_mxou_image_api(
            model=get_image_model("multi_angle"),
            token=token,
            prompt=prompt,
