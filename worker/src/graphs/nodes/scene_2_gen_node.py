...
def scene_2_gen_node(state: Scene2Input, config: RunnableConfig, runtime: Runtime[Context]) -> Scene2Output:
    ...
    # ⚠️ v0.31: 提示词走 prompt_assembler；slot_scene_context=scene_context_2 保证三张场景图差异化
    _vv = extract_visual_vars_from_draft(draft or {})
    prompt = assemble_prompt(
        "scene_2",
        title=title,
        scene_context=scene_context_2,
        slot_scene_context=scene_context_2,
        **_vv,
    )
