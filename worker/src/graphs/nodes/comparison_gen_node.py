...
def comparison_gen_node(state: ComparisonInput, config: RunnableConfig, runtime: Runtime[Context]) -> ComparisonOutput:
    ...
    """
    ctx = runtime.context
    # ✅ T7b: image_gen_plan 前置条件（plan 无 comparison → 直接跳过，不调生图 API）
    if not slot_enabled(config, "comparison", state):
        logger.info("image_gen_plan 未选择 comparison，跳过生图")
    ...
    if not draft or not token:
        ...
        logger.info("comparison_gen: 入口前置缺失，跳过生图 (draft=%s, token=%s)",
    ...
    prompt = assemble_prompt("comparison", title=title, **_vv, color_preset=_cp)
