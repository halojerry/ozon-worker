...
def scene_1_gen_node(state: Scene1Input, config: RunnableConfig, runtime: Runtime[Context]) -> Scene1Output:
    ...
    """
    ctx = runtime.context
    # ✅ T7b: image_gen_plan 前置条件（plan 无 scene_1 → 直接跳过，不调生图 API）
    if not slot_enabled(config, "scene_1", state):
        logger.info("image_gen_plan 未选择 scene_1，跳过生图")
    ...
    if not draft or not token:
        ...
        logger.info("scene_1_gen: 入口前置缺失，跳过生图 (draft=%s, token=%s)",
    ...
    # slot_scene_context=scene_context_1 保证三张场景图差异化（F6，不覆盖）
    ...
    prompt = assemble_prompt(
        "scene_1",
        title=title,
        scene_context=scene_context_1,
        slot_scene_context=scene_context_1,
        **_vv,
        color_preset=_cp,
    )
