...
class MxouCallLedger(Base):
    ...
    批D v0.78（取证 I5）：成败观测——record_call 插入 outcome='pending'，
    mxou_api 在 POST 返回/异常处 finish_call 回写终态（ok/failed/config_error）
    + duration_ms（该模型段耗时毫秒）。行不删改、无重写（单向 pending→终态）；
    存量历史行由 init_data.migrate_ledger_outcome_v078 幂等迁移回填 'ok'
    （迁移时刻无法区分历史成败，按成功口径回填保持可读——批D 拍板口径）。
    ...
    outcome: Mapped[str] = mapped_column(
        ...
        comment="调用结局（批D v0.78）：pending=进行中/未回写 → ok/failed/config_error；历史行迁移回填 'ok'")
    duration_ms: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True,
        comment="该模型段耗时毫秒（finish_call 回写；可空=未知）")
