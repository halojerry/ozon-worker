...
### ⚠️ 系统性根因(2026-08-08 本地实测「套娃」错配实证, 未来所有产品适用)

**根因: Ozon 字典属性是「值列表」而非自由文本, 多值列表的第一个值语义随机**
——同大类下所有 type 小类都出现在 8229 值列表里(如「纪念品和礼品」大类 → 手持风扇
的 8229 列表含「套娃/纪念品/手持风扇」18 项)。任何「取第一个字典值」的兜底都会错配。

**机制: 8229(类型)的字典值 id == type_id 本身**(类目 type 节点就是 8229 的一个值,
实测手持风扇 148495146 / 杀虫剂 99385)。因此 8229 根本不需要搜索/匹配:
**直接用 type_id 从 8229 字典值里查 id == type_id 那条**。

**通用纪律(改属性兜底前必读)**:
1. **8229 = type_id**: 所有场景(assemble/retry/prepare)优先按 type_id 匹配, 绝不取第一个
2. **字典属性绝不盲补首值**: 兜底顺序 = 语义匹配(attr_defaults) → 标题 2-gram 子串
   → 唯一值 → **None(宁缺毋滥, 交 Ozon 报可修复错)**。已修: 8229(type_id)/
   9782(安全默认)/4958(标题搜索)/9163(性别语义)
3. **values/search 是「按文本搜索」不是「取任意值」**: 命中多个时优先精确/标题相关,
   单值属性取精确命中, 无精确则跳过(不取第一个)
4. **类目匹配是上游根因**: pg_trgm 阈值 0.3 偏低, 类目错配 → 8229/属性全错。
   低置信度(sim < 0.5)应警告人工确认
