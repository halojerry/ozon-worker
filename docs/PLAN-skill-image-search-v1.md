...
**需求 3【部分解决，深度歧义待 LLM 消歧】类目树歧义**：
- 已解决：末级词净化 + RU fallback（门铃 RU"дверной звонок"正确命中 88265327/94537）
- 未解决（深度歧义）：护手霜→Крем интимный、粉扑→丙烯酸粉单体、化妆品收纳盒→Органайзер рыболовный——RU 翻译本身歧义，Ozon 树按词匹配到错误类目
- 真实基线：10 商品 4/10 精确命中（陀螺+3 化妆刷），含 RU 兜底可到 6/10
- **待实现方案**：`search_categories_validated` 返回多候选时，用 LLM 判定候选与"1688 末级词中文语义"的相关性（候选里选"收纳/化妆"语义匹配，而非"крем"字面匹配）——涉及 LLM 成本与准确率权衡，独立优化项

**Issue 6 剩余【待实现】search 增强**：蓝海评分（`calculate_blue_ocean_score`）/ Ozon 匹配度 / `--rules` 筛选 / `--auto-submit` 批量上架——利润估算/排序/导出已实现（commit 1ee991f）
