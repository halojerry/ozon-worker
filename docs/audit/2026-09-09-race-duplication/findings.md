...
- **连带发现修正（3939e109 树刷新实施后）**：原判「类目树数据过期」**不成立**——refresh 脚本 dry-run 实证 added=0（旧表已含现役树全部 type；双语各 upsert 7933、软失效 59 个 Ozon 下架 type、L0 引用 0、幂等验证过、worker 全量 2331 绿）。面包屑未命中的真因有二：①**双命名体系**——Web 前台面包屑与 Seller 树路径组织/措辞均不同（Термосы 子树在 Seller 树=Дом и сад > Термосы, термокружки…，dc=17027928），path-only 精配对 Web 面包屑天然不命中；②**官方 ZH 译名与中文卖家词不对齐**——现役保温杯类目官方译名「保暖杯/热水瓶」，jieba 搜「保温杯」零召回实证，skill graph 曾猜对 dc=17027928 却被自校验闸字符串比较错杀。**②是历史类目匹配低置信/错配的最深层根因**（见 F-B04）。
- 类型：drift/data；严重度：中；状态：断点已修，树重导待运维排期

### F-B04  官方 ZH 译名与中文卖家词不对齐——jieba 类目搜索零召回 + skill 自校验闸错杀正确猜测
- 类型：data/drift；严重度：**高**（类目匹配低置信/错配的最深层根因）；状态：**已修复（70e51dec）**
- 证据：现役保温杯类目 dc=17027928 官方 ZH 译名叶「保暖杯」(tp92574)/「热水瓶」(tp92576)——jieba `search_nodes("保温杯")` **零召回**实证（树刷新后复测）；skill graph 曾猜对 dc=17027928（信封名'保暖杯'）却被 `_category_guess_consistent` 以「1688 来源类目'保温杯'≠猜测名'保暖杯'」字符串比较丢弃——正确猜测被错杀，被迫走全量文本链再低置信。
- 状态更新（2026-09-09 用户批准四刀全做，70e51dec）：**已修复**——①jieba 搜索同义词扩展（config/category_synonyms.json 热加载，'保温杯'→['保暖杯','热水瓶','保温瓶']；同义词进 core_tokens 防被 node_name 约束整体剔除——首版漏加实证候选被剔成 0），零召回根治（召回 3 条 top-1 即正确类目）；②skill 自校验闸：R1 加 CJK 单字回退（一字之差 bigram 全不同但单字重叠 2/3）+ search_categories CJK 单字兜底 + max_results 1→3 多候选逐个过闸（graph 端到端实证：信封现带 dc=17027928/tp=92574 正确类目，不再错杀；毒类目金属管仍被 R2 尾字拦）；③get_node_by_full_path 末段退化加跨树守卫（候选 full_path 须含查询父类目段）；④web_category_path_map 新表：learning approved 回填（source=page 守卫）+ assemble 读侧 path-only hint 查映射直通（hit_count 原子递增，未命中回落树精配→文本链）。skill 1001 绿 / worker 2353 绿。
