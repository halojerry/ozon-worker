...
- 类型：data/drift；严重度：**高**（类目匹配低置信/错配的最深层根因）；状态：待修（2026-09-09 树刷新实施中发现）
...
- 修复方向：①类目搜索加同义词扩展（保温杯→保暖杯|热水瓶|термос）或 ZH 译名对齐表；②skill 自校验闸改「猜测 dc/tp 树存在性校验」替代字符串相等（猜对即放行复核，猜错才弃）；③`get_node_by_full_path` 末段退化匹配加路径前缀相似度守卫（已观测同名叶跨树错配：Спорт 措辞路径命中 Дом и сад 的 Термос）。
