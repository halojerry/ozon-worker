...
- [ ] `agent_created: true` 加入 frontmatter(WorkBuddy SkillManage 可管理)
- [ ] description 改第三人称客观描述(当前仍条件句):
  ```yaml
  description: >
    ...
    以图搜款、趋势选品全流程。
  ```
- [ ] version 保持 `"0.27.1"`(Codex PRD 写 0.27.0 已过时)
...
**B3 compile.py**:
- [ ] `DOC_FILES` 加 4 个 references/ 路径(纯列表追加,不改编译逻辑)

**B4 打包验证**:
- [ ] `python3.12 compile.py` 后 dist 含 references/ 4 文件(CI 会做,本地 darwin-arm64 验证)

**风险缓解**:速查表自包含核心参数;SKILL.md 明确「完整参数见 references/command-reference.md」;拆后跑全量测试 + import 验证。

### Phase C — 祈使句全文改写(中)

- [ ] SKILL.md + references/ 全文第二人称 → 祈使句/客观描述
- [ ] 保留规则(Codex PRD §4.2):
  - 「告诉用户」= 对 agent 的指令,保留
  - 引号内用户原文(如"帮我选品")保留
  - 「Role: operator. Execute CLI commands to complete tasks.」式开场
- [ ] 当前残留少(仅 1 处「你」模式),重点检查 references/ 拆出后的表述

**验收**:grep 无「你只需/你的角色/你用以下」(引号内用户原文除外)。

### Phase D — 跨 Agent 兼容(小)

- [ ] §0 新增「定位 Skill 目录」说明(SKILL_DIR / 当前目录 / 上级查找)
- [ ] `python3.12` → `python3`(38 处,SKILL.md + references/),注明 Python ≥ 3.12
- [ ] 不修改 cli.py(shebang 已是 `#!/usr/bin/env python3`)

**验收**:SKILL.md 无 `python3.12` 硬编码(除版本要求说明)。

### Phase E — 意图路由补「批量处理」分支(小)

- [ ] 决策表加分支:`多个 URL / "批量处理这些" → 管线 F batch_test`(当前只有「多 URL 追问」,补显式批量触发)
- [ ] 保留现有增强(URL 类型/蓝海/指代/降级),**不退回 Codex 的 7 分支**

### Phase F — 同步与回归(收尾)

- [ ] deploy/skill 同步(rsync,排除 .1688-AK/data/tests/dist)
- [ ] skill 全量测试(.venv314 pytest tests/)
- [ ] compile.py DOC_FILES 完整性核对(4 references 文件在列表)
- [ ] git 提交拆分:docs(skill) frontmatter+references / refactor(skill) 祈使句 / docs(skill) python3+管线F
- [ ] push dev
...
- [ ] frontmatter 4 项合规(name/description/version/agent_created)
- [ ] SKILL.md ≤ 160 行(骨架);references/ 含 4 文件
- [ ] dist 打包含 references/(compile.py DOC_FILES 核对)
- [ ] 无 python3.12 硬编码(除版本要求);§0 目录定位说明在
- [ ] 意图路由含批量分支 + 全部现有增强
- [ ] skill 全量测试通过(81+);deploy/skill 一致;无凭证泄漏
